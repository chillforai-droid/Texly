...
export const ToolPage: React.FC = () => {
  ...
  useEffect(() => {
    if (tool) {
      ...
      const secondaryKeywords = tool.secondaryKeywords || [];
      
      document.title = `${tool.name} – Free Online Tool | Texly`;
      const metaDesc = document.querySelector('meta[name="description"]');
      if (metaDesc) {
        metaDesc.setAttribute('content', `${tool.name}: Use our free ${primaryKeyword} to clean text online free and fix messy text online instantly. No signup, 100% secure text cleanup tool online.`);
      }

      // JSON-LD
      const schemas = getJSONLD(tool.name, tool.slug, primaryKeyword);
  ...
  const handleOcr = async () => {
    ...
    try {
      ...
      setOutput(result.data.text || 'No text found.');
    } catch (error) {
      console.error('OCR Error:', error);
      setOutput('Error extracting text. Please try again with a clearer image.');
    } finally {
      setLoading(false);
    }
  };
              ...
              <div className="grid grid-cols-1 gap-8">
                ...
                <div className="space-y-4">
                  ...
                  {tool.id === 'invisible-text' && (
                    <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-xl border border-slate-100">
                      <label htmlFor="invisible-count" className="text-sm font-bold text-slate-700 uppercase tracking-wider">Character Count:</label>
                      <input 
                        id="invisible-count"
                        type="number" 
                        className="w-24 p-2 bg-white border border-slate-200 rounded-lg font-bold text-blue-600"
                  ...
                  {tool.id === 'add-prefix' && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 bg-slate-50 rounded-xl border border-slate-100">
                      <div>
                        <label htmlFor="prefix-text" className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Prefix:</label>
                        <input 
                          id="prefix-text"
                          type="text" 
                      ...
                      <div>
                        <label htmlFor="suffix-text" className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Suffix:</label>
                        <input 
                          id="suffix-text"
                          type="text" 
                          className="w-full p-3 bg-white border border-slate-200 rounded-lg"
                  ...
                  {tool.id === 'text-steganography' && (
                    <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-xl border border-slate-100">
                      <label htmlFor="steg-mode" className="text-sm font-bold text-slate-700 uppercase tracking-wider">Mode:</label>
                      <select 
                        id="steg-mode"
                        className="p-2 bg-white border border-slate-200 rounded-lg font-bold text-blue-600"
                  ...
                  {tool.id === 'password-gen-strength' && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 bg-slate-50 rounded-xl border border-slate-100">
                      <div>
                        <label htmlFor="pass-mode" className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Action:</label>
                        <select 
                          id="pass-mode"
                          className="w-full p-3 bg-white border border-slate-200 rounded-lg font-bold text-blue-600"
                      ...
                      {options.mode === 'generate' && (
                        <div>
                          <label htmlFor="pass-length" className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Length:</label>
                          <input 
                            id="pass-length"
                            type="number" 
                            className="w-full p-3 bg-white border border-slate-200 rounded-lg"
                  ...
                  {tool.id === 'text-diff' && (
                    <div className="p-4 bg-blue-50 rounded-xl border border-blue-100">
                      ...
                      <p className="text-sm text-blue-800">Use the format:<br/><strong>Original:</strong> [your text]<br/><strong>Modified:</strong> [your text]</p>
                    </div>
                  )}
