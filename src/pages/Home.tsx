    ...
    <main className="relative min-h-screen">
              ...
              <div key={cat.id} className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-all hover:-translate-y-1" role="listitem">
                ...
                <div className="space-y-2">
                  {ALL_TOOLS.filter(toolItem => toolItem.category === cat.id).slice(0, 3).map(toolItem => (
                    ...
                    >
                      <span>{t.toolNames[toolItem.id] || toolItem.name}</span>
            ...
            >
              <div className="flex items-start justify-between mb-6">
                ...
                <div className="flex flex-col items-end gap-2">
                  ...
                  {idx < 3 && !searchQuery && (
                    <span className="text-[10px] font-bold text-amber-500 uppercase tracking-widest">{t.home.popular}</span>
                  )}
                </div>
              </div>
              <h3 className="text-xl font-bold text-slate-900 mb-3 group-hover:text-blue-600 transition-colors">{t.toolNames[tool.id] || tool.name}</h3>
              <p className="text-slate-500 line-clamp-2 mb-8 flex-grow leading-relaxed">{t.toolDescriptions[tool.id] || tool.shortDescription}</p>
              <div className="flex items-center justify-between pt-4 border-t border-slate-50">
                ...
                <span className="text-[10px] font-medium text-slate-400">{t.home.freeAndSecure}</span>
              </div>
            </Link>
          ))}
        </div>
      ...
      <section className="mt-24 mb-16" aria-labelledby="video-title">
        <div className="max-w-4xl mx-auto text-center">
          <h2 id="video-title" className="text-3xl font-bold text-slate-900 mb-4">{t.home.featuredVideo}</h2>
          <p className="text-lg text-slate-600 mb-10">{t.home.videoDesc}</p>
          <div className="relative mx-auto max-w-[350px] aspect-[9/16] bg-slate-100 rounded-3xl overflow-hidden shadow-2xl border-8 border-slate-900">
             <iframe 
