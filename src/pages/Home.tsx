    ...
    <main className="relative min-h-screen">
      ...
      <section className="text-center mb-16" aria-labelledby="hero-title">
        ...
        <div className="relative max-w-xl mx-auto group">
          ...
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 w-5 h-5 group-focus-within:text-blue-500 transition-colors" aria-hidden="true" />
          <input
            id="tool-search"
            type="text"
              ...
              <div key={cat.id} className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-all hover:-translate-y-1" role="listitem">
                ...
                <div className="space-y-2">
                  {ALL_TOOLS.filter(t => t.category === cat.id).slice(0, 3).map(t => (
                    ...
                    >
                      <span>{t.name}</span>
                      <ArrowRight className="w-3 h-3 opacity-0 -translate-x-2 group-hover/link:opacity-100 group-hover/link:translate-x-0 transition-all" aria-hidden="true" />
                    </Link>
                  ))}
                </div>
              </div>
