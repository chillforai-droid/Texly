export const getSEOContent = (toolId: string, toolName: string, keyword: string) => {
  return `
    <article class="prose prose-slate max-w-none">
      <section class="mb-12">
        <h2 class="text-2xl font-bold text-slate-900 mb-4">Need to ${keyword}? We've got you covered!</h2>
        <p>Welcome to <strong>Texly</strong>, your friendly neighborhood for simple text tools. If you've ever struggled with messy text, weird formatting, or just need a quick fix, you're in the right place. Our <strong>${toolName}</strong> is built to be fast, easy, and completely free.</p>
        <p>We know how annoying it is to do repetitive tasks by hand. That's why we made this tool—to save you time and keep your work clean. No signups, no hidden fees, just pure utility right in your browser.</p>
      </section>

      <section class="mb-12">
        <h2 class="text-2xl font-bold text-slate-900 mb-4">What is the ${toolName}?</h2>
        <p>The <strong>${toolName}</strong> is a simple online tool that helps you ${keyword} in seconds. Whether you're dealing with a single sentence or a massive document, our tool processes it instantly. It's perfect for anyone who works with text daily—from students to developers.</p>
        <p>Unlike some other sites, we don't store your data. Everything happens right here in your browser, so your text stays private and secure. It's like having a personal text assistant that's always ready to help.</p>
      </section>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <div class="p-3 bg-white border border-slate-200 rounded-lg text-sm font-mono text-slate-500 italic">
              Messy text with ${keyword} issues...
            </div>
          </div>
          <div>
      ...
      <section>
        <h2 class="text-2xl font-bold text-slate-900 mb-4">Ready to get started?</h2>
        <p>Give the <strong>${toolName}</strong> a try right now! Just paste your text above and see the magic happen. If you find it useful, don't forget to bookmark us and share it with your friends. Happy text fixing!</p>
      </section>
    </article>
  `;
};
