            ...
            <div className="absolute right-0 top-full pt-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all" role="menu">
              <div className="bg-white border border-slate-200 rounded-xl shadow-xl p-4 w-64 grid grid-cols-1 gap-2">
                {ALL_TOOLS.slice(0, 10).map(toolItem => (
                  <Link key={toolItem.id} to={`/tool/${toolItem.slug}`} className="text-sm text-slate-500 hover:text-blue-600 hover:bg-blue-50 p-2 rounded-lg transition-all" role="menuitem">
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 flex flex-col">
      ...
      <main className="flex-grow">
        <Routes>
          ...
          <Route path="/admin-9823hdjfsdf" element={<AdminPage />} />
                  ...
                  <ul className="space-y-6" role="list">
                    {categoryTools.map(toolItem => {
                      ...
                      return (
                        <li key={toolItem.id}>
                          <Link 
                            to={`/tool/${toolItem.slug}`} 
                            className="group block"
                          >
                            <div className="flex items-center justify-between mb-1">
                              <div className="flex items-center gap-2">
                                <ToolIcon className="w-4 h-4 text-slate-400 group-hover:text-blue-600 transition-colors" />
                                <span className="font-bold text-slate-800 group-hover:text-blue-600 transition-colors">{t.toolNames[toolItem.id] || toolItem.name}</span>
                              </div>
            ...
            <div>
              ...
              <ul className="space-y-4">
                {ALL_TOOLS.filter(toolItem => toolItem.category === 'cleaning').slice(0, 5).map(toolItem => (
                  <li key={toolItem.id}><Link to={`/tool/${toolItem.slug}`} className="text-sm text-slate-500 hover:text-blue-600">{t.toolNames[toolItem.id] || toolItem.name}</Link></li>
                ))}
              </ul>
            </div>

            <div>
              <h4 className="font-bold text-slate-900 mb-6 uppercase text-xs tracking-widest">{t.footer.converters}</h4>
              <ul className="space-y-4">
